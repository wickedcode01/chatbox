...
export default class Claude extends Base {
    ...
    createMessage(model: string, max_tokens: number, messages:Message[]|object[], system: string, tools?: Tool[]) {
        return this.client.messages.stream({
            model,
            max_tokens,
            messages: (messages as Message[]).map(({ content, role }) => ({ content, role: role === 'system' ? 'user' : role })),
            system,
            tools
        })
    }
        ...
        return new Promise((reslove, reject) => {
            ...
            stream.on('text', (text: string) => {
                ...
                if (searchTag && search_query.length > 0) {
                    const results = await Promise.all(search_query.map(async query => ([... await performSearch(query)]))).catch(err => {
                        reject(err);
                    })
                    ...
                    const searchStream = await this.createMessage(
                        ...
                        this.maxTokens,
                        [...messages, {
                            role: 'user', content: "You are an AI assistant tasked with answering user questions based on provided search results. Use the search results to create an accurate and concise answer. Include in-text citations for references, linking directly to the sources. Adjust the answer's language to match the user's query language." 
                    ...
                    searchStream.on('text', (text: string) => {
                        result += text
                        onResultChange?.(result)
                        reslove(result)
